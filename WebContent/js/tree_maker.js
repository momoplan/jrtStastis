/************************************************************************************
*Title   :   treeMaker 脚本文件 (Ver 3.0.3)
*program :   区钊贤
*website :   http://www.ouzx.com
*e-mail  :   ouzhaoxian@21cn.com
*Date    :   2003-06-09  (all right reserved)
************************************************************************************/
var Tree_CHILD=-1;
var Tree_LAST=-2;
var Tree_ROOT=-3;
...
{
	this.showLine=false;
	this.lineFolder="../images/";//连线图片文件夹
	this.Indent=13;
	...
	this.expand=function expand(nodeID,isExpand){
		...
		var td=document.getElementById("Tree_td_"+nodeID);
		if(node){
			if(node.expanded==isExpand){this.flag=false;return;}		
			if(td){
				td.onclick();	
...
function Tree_on_action(action)//fire when click node
{
	if(action.type==Tree_LINK)//link
	{
		...
		if( (target=="_self"||target=="_top"||target=="_parent"||target=="_blank" ) && action.link!="")
